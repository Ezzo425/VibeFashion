...
RUN apt-get update \
    && apt-get install -y --no-install-recommends libsqlite3-dev libonig-dev unzip \
...
COPY docker/entrypoint.sh /usr/local/bin/vibefashion-entrypoint
RUN chmod +x /usr/local/bin/vibefashion-entrypoint

EXPOSE 80

ENTRYPOINT ["vibefashion-entrypoint"]
