function valid_datas(f) {
    if (f.name.value == "") {
        ...
        jQuery.ajax({
            url: "mail.php",
            type: "post",
            data: jQuery("form#vibefashion-contact").serialize(),
            complete: function (data) {
                jQuery("#form_status").html(data.responseText);
                jQuery("#vibefashion-contact")
                    .find("input,textarea")
                    .attr({ value: "" });
                jQuery("#vibefashion-contact").css({ opacity: 1 });
                jQuery("#vibefashion-contact").remove();
            },
        });
        jQuery("#form_status").html(
            '<span class="loading">Sending your message...</span>',
        );
        jQuery("#vibefashion-contact").animate({ opacity: 0.3 });
        jQuery("#vibefashion-contact")
...
function notice(f) {
    jQuery("#vibefashion-contact").find("input,textarea").css("border", "none");
